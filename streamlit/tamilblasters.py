...
class Tamilblasters():
    ...
    def movie_search(self, query):
        ...
        current_url = db_handler.get_current_url("tamilblasters") #gets the current url

        current_url=web_utils.get_url(current_url) #checks if current url is working

        url=db_handler.update_url_if_needed("tamilblasters",current_url) #captures any changes in the url domain

        if Tamilblasters.count_forward_slashes(current_url)>3:
            url=Tamilblasters.remove_extra_url(current_url) #ensure always returns the home page of the url.

        dicto = {}
        search_url = url + f"index.php?/search/&q=\"{query.replace(' ', '%20')}\"&updated_after=any&sortby=relevancy&search_in=titles"
    ...
    def stream_link_fetcher(self, selected_movie_link):
        ...
        if tree is None:
            print("Error getting the webpage")
            return ""
        magnet_link=tree.xpath('//a[@class="magnet-plugin"]/@href')
        if magnet_link:
            return magnet_link
        else:
            return ""
